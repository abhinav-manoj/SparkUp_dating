...
def create_profile(request):
    ...
    if request.method == "POST":
        ...
        if profile_form.is_valid() and formset.is_valid():
            ...
            # Handle location data directly
            location = request.POST.get('location', '')
            citylat = request.POST.get('citylat', '')
            citylong = request.POST.get('citylong', '')
            
            if location and citylat and citylong:
                instance.location = location
                instance.citylat = citylat
                instance.citylong = citylong
            
            instance.save()
            
            # Rest of your existing code for handling images
            ...
            for form in formset:
                if form.is_valid() and form.has_changed():
                    ...
                    instance_image.image.name = 'your_desired_name.jpg'
                    instance_image.save()
    ...
    context = {
        ...
        'citylat': request.user.profile.citylat if hasattr(request.user, 'profile') else '',
        'citylong': request.user.profile.citylong if hasattr(request.user, 'profile') else ''
    }
